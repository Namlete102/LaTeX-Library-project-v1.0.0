...
**Lorem ipsum** là một [đoạn văn bản giả hoặc văn bản giữ chỗ](https://en.wikipedia.org/wiki/Placeholder_text "Văn bản giữ chỗ") (d) thường được sử dụng trong thiết kế đồ họa, xuất bản và phát triển web. 
...
Để sử dụng được đoạn văn bản giả Lorem ipsum ở trong LaTeX, trước tiên người dùng cần phải khai báo Package sau được đặt ở . . . (vị trí nào trong phần soạn thảo ?): 
...
> [!WARNING]  
> Nếu như người dùng không viết phương trình toán có các [kí hiệu, chữ cái toán học]() đó vào một trong ba cách viết `inline math` trên, thì hệ thống sẽ báo lỗi `! Missing $ inserted.`  Chú  ý này cũng được áp dụng tương tự với việc người dùng viết toán học ở chế độ [`display math`]().
...
Tương tự như `inline math`, tùy thuộc vào sở thích, thói quen cá nhân của người dùng để có thể chọn một trong ba cách đầu tiên trên để viết `display math`, mà vẫn đảm bảo kết quả cho ra được vẫn là giống nhau. 

Ví dụ về việc viết `display math` bằng ba cách trên với phương trình logarit của một tích là tổng của logarit của các thừa số: :
...
Trong một số trường hợp, khi người dùng viết tài liệu toán học cần [tham chiếu]() đến phương trình toán học ở chế độ `display math` mà không cần phải viết lặp lại hoặc sẽ bị tràn ra khỏi văn bản (đối với việc viết phương trình dài (?)) của phương trình đó ở chế độ `inline math` . . . (? viết tiếp đê)    

Khác với ba cách ở trên, khi người dùng viết toán học ở chế độ `display math`, nếu như người dùng thay các cặp dấu, môi trường ở ví dụ . . . trên bằng một môi trường mới là môi ytuongfw `equation`, thì người dùng có thể thấy rằng phần toán học được đặt bên trong môi trường đó sẽ được LaTeX tự động đánh thêm số thứ tự bên cạnh phần toán học đó ở trong trang tài liệu 
...
Để tham chiếu chéo đến phương trình đó, người dùng cần sử dụng lệnh

```latex
\label{} 
```

được đặt ở trong môi trường `equation`. Và lệnh 
...
được đặt ở trong đoạn văn mà người dùng muốn, để tham chiếu đến phương trình có lệnh `\label` tương ứng. 

Điểm khác biệt của lệnh `\ref` và `\eqref` đó là lệnh `\eqref` sẽ tự động đóng ngoặc tròn `()` số thứ tự tương ứng với phương trình được tham chiếu, còn lệnh `\ref` thì không. Để dễ hình dung, người viết xem qua đoạn mã sau   
...
. . . (\ref{eq:1}) % lệnh \ref không được hệ thống tự động đóng ngoặc tròn được, mà người dùng phải tự thêm vào    
. . . \eqref{eq:1} . . .  
...
> [!NOTE]  
> Nội dung bên trong dấu ngoặc nhọn `{}` của lệnh `label` và lệnh `\ref` (hoặc `\eqref`) đều phải được giống nhau để hệ thống không báo lỗi . . .
...
Trước tiên người dùng cần phải khai báo` package`

```latex
\usepackage{amsmath}
```

sau đó người dùng chỉ cần thêm dấu `*` vào bên trong lệnh môi trường `equation` là phương trình logarit ở ví dụ . . . sẽ không còn xuất hiện số thứ tự bên cạnh ở trong trang tài liệu nữa:  
...
Tham khảo trang 70 tài liệu đây: **Một tài liệu ngắn gọn giới thiệu về LATEX 2ε - hay LATEX 2ε trong 155 phút**,  https://www.cmor-faculty.rice.edu/~heinken/latex/symbols.pdf
